...
function run-backup {
    (
        ...
        get-sensor
        update-sensor "${AWS_BACKUP_STATUS[1]}"

        # run entire backup steps
        # shellcheck disable=SC2015
        create-backup && copy-backup && cleanup-backups-local \
            && update-sensor "${AWS_BACKUP_STATUS[2]}" "ALL" \
            || update-sensor "${AWS_BACKUP_STATUS[3]}" "ALL"

        sleep 10
        update-sensor "${AWS_BACKUP_STATUS[0]}"
        bashio::log.info "Backup finished"
    ) 200>/tmp/aws_backup.lockfile
}
...
if [ "$SKIP_PRECHECK" = true ]; then
    update-sensor "${AWS_BACKUP_STATUS[0]}"
elif ! aws-precheck; then
    update-sensor "${AWS_BACKUP_STATUS[3]}"
    exit 1
else
    update-sensor "${AWS_BACKUP_STATUS[0]}" "ALL"
fi

bashio::log.info "AWS Backup started successfully"
