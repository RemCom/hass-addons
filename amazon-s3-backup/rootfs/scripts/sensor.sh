...
declare AWS_BACKUP_STATUS=(IDLE RUNNING SUCCEEDED FAILED)
declare SENSOR_NAME="sensor.aws_backup_backup"
declare SENSOR_URL="/core/api/states/${SENSOR_NAME}"
declare STORAGE_FILE="/backup/.aws_backup_backup.sensor"
...
function update-sensor {
    ...
    if bashio::var.has_value "${all}"; then
        ...
        if response=$(eval "${SMB} -c 'cd \"${TARGET_DIR}\"; ls'"); then
            ...
            if response=$(echo "$response" | grep -E '\<([0-9a-f]{8}|AWS_Backup_.*)\.tar\>'); then
        ...
        if [ "$CURRENT_STATUS" = "${AWS_BACKUP_STATUS[2]}" ]; then
            TOTAL_SUCCESS=$((TOTAL_SUCCESS + 1))
            LAST_BACKUP=$(date +'%Y-%m-%d %H:%M')
        elif [ "$CURRENT_STATUS" = "${AWS_BACKUP_STATUS[3]}" ]; then
